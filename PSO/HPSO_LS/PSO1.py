...
class PSO:
    ...
    def fiterOfSU(self, extratRatio):
        tempArray = []
        tempArray.append(range(0, len(self.dataFeature)))
        tempArray.append(list(self.suArray))
        ...
        delectRatio = 1 - extratRatio  # delectRatio = 0.6
        self.featureIndex = allIndex[int(len(self.suArray) * delectRatio):]
        # 删除该数组中前 delectRatio*len(self.pheromone)个特征
        newArray = tempArray[:, int(len(self.suArray) * delectRatio):]
        # 将类标签赋值给中间变量，用于最后添加上去
        # 将类标签添加回上去
        self.dataFeature = np.asarray(newArray[0])
        # 将信息素矩阵传回上去
        self.suArray = np.asarray(newArray[1])
        splie = np.asarray(self.dataFeature, dtype='int')
        self.dataX = self.dataX[:, splie]
    ...
    def localSearch(self,particle_i):
        ...
        # acc = self.fitnessFunction_CV(feature_x,feature_y)
...
if  __name__ == "__main__":
    ...
    for file in files:
        ...
        # 写入文件的路径
        path_txt = "D:/MachineLearningBackUp/实验/result_txt/HPSO_LS/" + ducName + ".txt"
        # 写入文件的标题
        ...
        with open(path_txt, 'a') as f:
            f.write(titleTxt)
            f.close()
        # path = "D:\MachineLearningBackUp\dataCSV\dataCSV_high" + "\\" + file
        # print("获取文件数据", file)
        dataCsv = ReadCSV(path=path_csv)
        print("获取文件数据",file)
        dataCsv.getData()
        pso = PSO(dataCsv=dataCsv, dataX = dataCsv.dataX ,dataY=dataCsv.dataY,dataName=file.split('.')[0])
        ...
        for i in range(iterateT):
            pso.runOfPSO()
            acc[i] = pso.globalBestFitness
            length[i] = len(np.where(pso.globalBestSolution == 1)[0])

            pso.globalBestFitness = 0
            pso.globalBestSolution = np.asarray([])
