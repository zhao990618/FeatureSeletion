...
class GRmoea:
    ...
    def featureOfSimilar_high(self):
        ...
        allFeatureSimilar = readSimilar(path="/home/fanfan/dataCSV/dataCSV_similar/"+self.dataName+"Similar.txt")
...
# 每过 self.grTime次迭代就要进行一次filter和wrapper之间的相互促进
        ...
        self.global_acc = np.append(self.global_acc, self.globalBestFitness)
        self.global_len = np.append(self.global_len, self.globalBestLength / len(self.dataFeature))
    ...
    def run(self):
        ...
        tempPop = self.deleteDuolicate(conPop=tempPop)
        # self.globalArchive = getParato(pop=tempPop,popNum=self.numberOfChromosome*2,dataFeatureNum = len(self.dataFeature))
        self.globalArchive = getPF(pop=tempPop)

if __name__ == "__main__":
    import time
    # files = os.listdir("/home/fanfan/GRMOEA/dataCSV")
    ...
    for file in files:
        ...
        grmoea.setParameter(crossover=0.9, mutation=0.1, numberOfChromosome=50, iteratorTime=200, gtTime=10)
        ...
        # 获得最终PF
        # ArchivePF = deleteDuolicate(conPop=ArchivePF)
        # 写入文件的路径
        global_pf = wb["GRMOEA"]
        global_pf.cell(row=1, column=1, value="acc")
        global_pf.cell(row=1, column=2, value="len")
        for i in range(len(grmoea.global_acc)):
            # 添加acc到xlsx
            global_pf.cell(row=2 + i, column=1, value = grmoea.global_acc[i])
            # 添加len到xlsx
            global_pf.cell(row=2 + i, column=2, value = grmoea.global_len[i])
