...
# ============================================================================================
    ...
    class Chromosome:
        ...
        def initOfChromosome_wrapper(self,GRmoea):
            #  高维数据，高维需要filter剔除数据，会把class剔除
            self.featureOfSelect = np.zeros(len(GRmoea.dataFeature))
            # 低维数据 ，不做filter提取数据，所以dataFeature包含class，所以需要减1
            #self.featureOfSelect = np.zeros(len(GRmoea.dataFeature) - 1)
            #得到wrapper种群的总个体
            numChromosome = int(GRmoea.numberOfChromosome)
            # 向上取整
            numberOfGroup = int((len(GRmoea.dataFeature))/numChromosome)
            ...
            # 使用{0，1}初始化
            # for i in range(0,len(self.featureOfSelect)):
            #     rand = random.random()
            #     if rand < 0.5:
...
# 每过 self.grTime次迭代就要进行一次filter和wrapper之间的相互促进
    ...
    def run(self):
        ...
        while runtime < self.iteratorTime:
            ...
            print(f"filter time = {time.time() - start} seconds")
            ...
            print(f"wrapper time = {time.time() - start} seconds")
        ...
        print("================================================")
        # print("最优个体solution为：", self.globalBestSolution)
        print("最优个体的fitness为：", self.globalBestFitness)
        print("最优个体的选择长度为：", self.globalBestLength)
        print(" ")

        #self.saveTrainSet.append(self.globalBestFitness)

        # acc = terminalComputeFitness(trainX=self.dataX, trainY=self.dataY
        #                              , testX=self.dataX_t, testY=self.dataY_t, solution=self.globalBestSolution)
        # #print("最优个体的选择长度为：", self.globalBestLength)
        # print(acc)
        # self.saveTestSet.append(acc)



if __name__ == "__main__":
    dataName = []
    dataCsv = ReadCSV(path="D:\MachineLearningBackUp\dataCSV\dataCSV_high\\arcene.csv")
    #dataCsv = DataCSV(path="D:\MachineLearningBackUp\data\ionosphere.csv")
    # dataCsv = DataCSV(path="D:\MachineLearningBackUp\dataCsv\CNS.csv")
    print("获取文件数据")
    dataCsv.getData()
    grmoea = GRmoea(dataCsv=dataCsv,  dataX=dataCsv.dataX, dataY=dataCsv.dataY)
    # crossover,mutation,numberOfChromosome,iteratorTime
...
##############################################################################################
    # # 默认分成10组
    # ss = StratifiedShuffleSplit(n_splits=10,train_size=0.9,random_state=42)
    # for train_index,test_index in ss.split(X=grmoea.dataX,y=grmoea.dataY):
    #     oriX = copy.deepcopy(grmoea.dataX)
    #     oriY = copy.deepcopy(grmoea.dataY)
    #     grmoea.dataX,grmoea.dataX_t = oriX[train_index],oriX[test_index]
    #     grmoea.dataY,grmoea.dataY_t = oriY[train_index],oriY[test_index]
    #     grmoea.run()
    #     # 重置
    #     grmoea.globalBestFitness = 0
    #     grmoea.globalBestSolution = np.asarray([])
    #     grmoea.globalBestFitness = 0
    #
    # for i in range(len(grmoea.saveTestSet)):
    #     print(grmoea.saveTrainSet[i]," ",grmoea.saveTestSet[i])
