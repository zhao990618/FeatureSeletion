...
class MOBGAAOS:
    ...
    def run(self):
        ...
        # 将非支配解放入到种群中去
        f_0 = getParato(pop=self.population_GA, popNum=self.populationNum)
        # 写入文件的路径
        path_txt = "../result/paretoSolution/MOBGA_AOS/" + self.dataName + ".txt"
        # 写入时间
...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        # 写入文件的路径
        path_txt = "../result/result_txt/MOBGA_AOS/" + ducName + ".txt"
        # 写入文件的标题
        titleTxt = '   ' + ducName + str(time.ctime()) + '.csv \n'
        # 将结果写入到文件中去
        with open(path_txt, 'a') as f:
            f.write(titleTxt)
            f.close()
        ...
        genetic.setParameter(populationNum=140, maxFEs=15000, crossoverPro=0.3,LP = 5,Q=5)
        # 循环多少次
        iterateT = 10
