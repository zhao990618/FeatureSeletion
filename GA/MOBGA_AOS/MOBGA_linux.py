...
class MOBGAAOS:
    ...
    def mutateOperator(self, individual):
        ...
        # 计算两个个体 fitness
        feature_x = self.dataX[:, individual.featureOfSelect == 1]
        tmp_u_len = len(np.where(individual.featureOfSelect == 1)[0])
        # 判断1的数量
        if tmp_u_len > 0:
            # 计算fit
            individual.mineFitness = 1 - fitnessFunction_KNN_CV(findData_x=feature_x,
    ...
    def run(self):
        ...
        # 运行次数
        self.stage_all()
        self.globalArchive = getPF(pop=self.population_GA)


if __name__ == '__main__':
    import time

    # files = os.listdir("/home/fanfan/MOBGA/dataCSV")
    ...
    for file in files:
        ...
        countNum = 0
        genetic = MOBGAAOS(dataX=dataCsv.dataX, dataY=dataCsv.dataY, dataName=ducName)
        for i in range(iterateT):
            genetic.setParameter(populationNum=100, maxFEs=20000, crossoverPro=0.3, LP=5, Q=5)
            # 保存最终的pf
            genetic.run()
            genetic.population_GA = np.asarray([])
        ...
        # 获得最终PF
        ArchivePF = genetic.deleteDuolicate(conPop=ArchivePF)
