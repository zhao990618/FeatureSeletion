...
import numpy as np
from dataProcessing.NonDominate import nonDominatedSort, sortFunction
# 开一个新空间
# from CMODE_SDE.CMODE1 import CMODESDE
# from MOBGA_AOS.MOBGA1 import MOBGAAOS
# from MOFS_BDE.BDE1 import MOFSBDE
# from NSGA_II.genetic1 import NSGA_II
# from mfmode.mfde1 import MFMODE
# from mfmode.Variant.mfdeVariant1 import MFMODE1
# from mfmode.Variant.mfdeVariant2 import MFMODE2
# from mfmode.Variant.mfdeVariant3 import MFMODE3

# 得到 帕累托 解
def getParato(pop,popNum,dataFeatureNum):
    dataFeature = np.arange(popNum)
    popErr = np.zeros(popNum)
    popLen = np.zeros(popNum)
    for i in range(len(pop)):
        popErr[i] = pop[i].mineFitness
        popLen[i] = pop[i].numberOfSolution
    ...
    errorArray, lenArray = sortFunction(error=popErr, solutionLength_test=popLen,dataFeatureNum = dataFeatureNum)
