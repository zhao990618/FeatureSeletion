...
class Genetic:
    ...
    def  setParameter(self,populationNum,iteratorTime,p,eta,c,mu_F,mu_CR,bm,rec):
    ...
    def getBestIndividualArray(self):
        ...
        self.global_acc = np.append(self.global_acc, errorArray[errIndex[0]])
        self.global_len = np.append(self.global_len, self.population_DE[errIndex[0]].proportionOfFeature)
    ...
    def getSubData(self):
        ...
        acc_ind = np.zeros(self.populationNum)
        for i in range(self.populationNum):
            acc_ind[i] = self.population_DE[i].mineFitness
        acc_index = np.argsort(acc_ind)

        conArray = [[]]
        conArray[0] = np.where(self.population_DE[acc_index[0]].featureOfSelect == 1)[0].tolist()

        for i in range(1, self.bm):
            # conArray.append(np.where(self.globalArchive[i].featureOfSelect == 1)[0].tolist())
            conArray.append(np.where(self.population_DE[acc_index[i]].featureOfSelect == 1)[0].tolist())
...
def creatXL(bm):
    ...
    for file in files:
        ...
        # 写入文件的路径
        path_xlsx = "/home/fanfan/result/result_txt/R_Compare/bm" + str(bm) + "/" + ducName +".xlsx"
        # 创建xlsx
...
if __name__ == '__main__':
    import time
    bm_array = np.asarray([1,5,10])
    # bm_array = np.asarray([10])
    for m in range(len(bm_array)):
        creatXL(bm=bm_array[m])
        ...
        for file in files:
            ...
            # 写入文件的路径
            path_xlsx = "/home/fanfan/result/result_txt/R_Compare/bm" + str(bm_array[m]) + "/" + ducName +".xlsx"
            # 创建xlsx
            ...
            for i in range(iterateT):
                ...
                genetic.setParameter(populationNum=100, iteratorTime=50, mu_F=0.5,
                                     mu_CR=0.5, eta=0.6, c=0.1, p=0.05, bm=bm_array[m], rec = 1)
                genetic.run()
