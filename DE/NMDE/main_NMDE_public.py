...
if __name__ == "__main__":
    ...
    dataset = tt[0]
    pop = np.asarray([])
    pop,first_rank_population,lenOfFeature = main(dataset)
    # index_non33 = first_nondominated(pop)#? 都没有用到，不知道为什么要加
    # folder1 = 'train' + str(dataset) + ".npy"
    ...
    print("acc=",1 - front[indexOfScore[0],0],"len=",
          front[indexOfScore[0],1] * lenOfFeature)
    fig1 = plt.scatter(front[:,1], front[:,0], color='r', lw=1.5,marker='o',ls= '-.')
    # fig2 =plt.scatter(f[:, 1], f[:, 0], color='k', lw=1.5,marker='+',ls= '-.')
    plt.grid(True)
    # plt.legend([fig1,fig2],['NMDE','NSGA_II'])
    plt.legend([fig1],['NMDE'])
    plt.axis("tight")
    plt.show()
