...
class Genetic:
    ...
    class Chromosome:
        ...
        def initOfChromosome(self,MFEA):
            ...
            acc = fitnessFunction_KNN_CV(findData_x=feature_x, findData_y=MFEA.dataY, CV=5)
    ...
    def mcs_DE(self):
        ...
        # 获得最优acc的三个个体
        errArray = np.zeros(self.populationNum)
        for i in range(self.populationNum):
            errArray[i] = self.population_DE[i].mineFitness
        # 通过err数组进行排序，升序排序
        errIndex = np.argsort(errArray)
        # 将输出前三个
        for i in range(3):
            print("acc = ",1 - self.population_DE[errIndex[i]].mineFitness,
                  "len = ",self.population_DE[errIndex[i]].numberOfSolution)
    ...
    def selectionOperation(self,u_x,individual_i):
        ...
        acc = fitnessFunction_KNN_CV(findData_x=feature_x, findData_y=self.dataY, CV=5)
        u_fit = 1 - acc
    ...
    def featureDominate(self,p_0,p_1,individual):
        ...
        err_0_0 =1 - fitnessFunction_KNN_CV(findData_x=feature_0_0,findData_y=self.dataY,CV=5)
        # 计算 0_1情况的acc
        feature_0_1 = self.dataX[:,x_new_0_1_f == 1]
        err_0_1 =1 - fitnessFunction_KNN_CV(findData_x=feature_0_1,findData_y=self.dataY,CV = 5)
    ...
    def oneBitPureSearch(self,population):
        ...
        for i in range(self.populationNum):
            ...
            copyS_Feature = self.dataX[:,copySolution == 1]
            copyS_Error = 1 - fitnessFunction_KNN_CV(findData_x=copyS_Feature,findData_y=self.dataY,CV=5)
...
if __name__ == '__main__':
    ...
    genetic.setParameter(populationNum=100,iteratorTime=200,crossoverPro=0.3,F=0.5,sigma = 0.01,pureTime = 5)
    genetic.run()