...
# 将非支配解放入到种群中去
        f_0 = getParato(pop=self.population_DE, popNum=self.populationNum)
        # 写入文件的路径
        path_txt = "../result/paretoSolution/MOFS_BDE/" + self.dataName + ".txt"
        # 写入时间
...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        # 写入文件的路径
        path_txt = "../result/result_txt/MOFS_BDE/" + ducName + ".txt"
        # 写入文件的标题
        ...
        genetic.setParameter(populationNum=140,iteratorTime=100,crossoverPro=0.3,F=0.5,sigma = 0.01,pureTime = 5)
        ...
        # 向文件中写入均值
        # 写入文件的值
        stringOfResult = str(acc.mean()) + '\t' + str(acc.std()) + '\t' + str(length.mean()) + '\n'
        # 将结果写入到文件中去
        with open(path_txt, 'a') as f:
            f.write('   mean  \n')
            f.write(stringOfResult)
        ...
        print("acc:", acc.mean(), "  std:", acc.std(), "  len:", length.mean())
