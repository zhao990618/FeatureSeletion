...
from mfmode.operators import mutate_best,mutate_rand1,mutate_rand2
from mfmode.operators import crossover_num
from mfmode.operators import getFreq,getAccRecMean
from mfmode.dataMemory import memory
...
class Genetic:
    ...
    # 历史最差种群
    worstPopulation = [[] for i in range(2)]
    # 每一个种群的数量
    populationNum = 0
    # 种群迭代次数
    iteratorTime = 0
    ...
    # 设置参数
    def setParameter(self, populationNum, iteratorTime, eliteProb,crossoverPro, F,p,eliteFront):
    ...
    def getNextPopulation(self, population ,task):
        ...
        # 输出前三个最优fitness对应的个体
        for k in range(3):
            print(1 - population[k].mineFitness," ",population[k].numberOfSolution," ",population[k].isNotRedundance
                  ," ",)
        print("获得精英代")
    ...
    def removeIrralevantFeature(self, population,task,ePro):
        ...
        for i in range(len(mrmrOfFeature)):
            ...
            for j in range(int(eachIndSubPopNum)):
                ...
                len1 = np.where(solution1_B == 1)[0].shape[0]
                # len2 = len(np.where(solution2_B == 1)[0])
                len2 = np.where(solution2_B == 1)[0].shape[0]
                # len3 = len(np.where(solution3_B == 1)[0])
                len3 = np.where(solution3_B == 1)[0].shape[0]
                ...
                solution_B = tempArray_B[tempIndex[0]]
                solution_N = tempArray_N[tempIndex[0]]
                error = 1 - tempArray_acc[tempIndex[0]]
    ...
    def mate_new(self,taskNum):
        ...
        for i in range(len(taskPopulation)):
            ...
            # 存放两个个体在整体大种群中的索引位置
            # index1 = mateIndex[i]
            # index2 = int(np.where(taskPopulation == indMain)[0])
            ...
            if (ind1.task == indMain.task):
                if ind1.task == 0:
                    ...
                    # tempRange = np.delete(temp1, [index1, index2]).tolist()
                    # tempInd = sample(tempRange,2)
                    # xArray[0] = self.population_task1[tempInd[0]]
                    # xArray[1] = self.population_task1[tempInd[1]]
                    tempA1 = np.delete(self.population_task1, [index1, index2]).tolist()
                    xArray = sample(tempA1, 2)

                elif ind1.task == 1:
                    # # 然后在该个体上删除 上面选择的 差分个体  和 基底个体
                    # tempRange = np.delete(temp1, [index1 - self.populationNum, index2 - self.populationNum]).tolist()
                    # # 需要将同一个任务的两个个体都删除，这样接下来的筛选就不会重复选到这两个个体了
                    # tempInd = sample(tempRange,2)
                    # xArray[0] = self.population_task2[tempInd[0]]
                    # xArray[1] = self.population_task2[tempInd[1]]
            ...
            # 赋值，通过三种条件
            solution_u[0] = np.where(u_x > 0.8,1,0)
            solution_u[1] = np.where(u_x < self.scoreOfRelifF,1,0)
            solution_u[2] = np.where(u_x < self.probabilityOfFeatureToSelect,1,0)
            ...
            for u in range(3):
                ...
                if solution_len[u] == 1:
                    ...
                    solution_acc[u] = fitnessFunction_KNN_CV(findData_x=featureX, findData_y=featureY, CV=10)
            ...
            if ind1_score <= solution_score[solution_index[0]]:
                # 跟新实数
                ind1.featureOfSelectNum = u_x
                # 跟新二进制
                ind1.featureOfSelect = solution_u[solution_index[0]]
                # 跟新error = 1 - acc
                ind1.mineFitness = 1 - solution_acc[solution_index[0]]
                # 更新数据
        ...
        # 非支配
        taskPop = [[] for i in range(2)]
        taskPop[0] = self.population_task1
        taskPop[1] = self.population_task2
        for i in range(2):
            springErr = np.zeros(self.populationNum)
            springLen = np.zeros(self.populationNum)
            for j in range(self.populationNum):
                springErr[i] = copyTask[i][j].mineFitness
                springLen[i] = copyTask[i][j].numberOfSolution

            taskErr = np.zeros(self.populationNum)
            taskLen = np.zeros(self.populationNum)
            for m in range(self.populationNum):
                taskErr[m] = taskPop[i][m].mineFitness
                taskLen[m] = taskPop[i][m].numberOfSolution

            # 将精英代添加到种群中去 np.vstack 添加到列 ；np.hstack 添加到行
            population = np.concatenate((copyTask[i], taskPop[i]), axis=0)
            saveFitness = np.concatenate((springErr, taskErr), axis=0)
            saveLength = np.concatenate((springLen, taskLen), axis=0)

            taskPop[i] = self.nonDominatedSort(parentPopulation=population,
                                               fitnessArray=saveFitness, solutionlengthArray=saveLength)
    ...
    # 得到特征被选择的频率
    def getFeatureFrequency(self,population):
        # 保存所选择特征的
        frequencyArray = [[] for i in range(self.populationNum)]
        accArray = np.zeros(self.populationNum)
        lenArray = np.zeros(self.populationNum)
        occAccLenArray = np.zeros(2)
        for i in range(self.populationNum):
            # 将二进制的solution进行一个保存
            frequencyArray[i] = population[i].featureOfSelect
            # acc
            accArray[i] = 1 - population[i].mineFitness
            # 未选择长度
            lenArray[i] = 1 - population[i].numberOfSolution/len(self.dataFeature)
        # 按照列相加,并且得到其比例
        freArray = getFreq(solution=frequencyArray)
        # 获得acc 和 len 的mean
        occAccLenArray[0] = accArray.mean()
        occAccLenArray[1] = lenArray.mean()
        return freArray,occAccLenArray

    # 探索
    def exploreStage(self, runTime):
        # 保存featureY
        featureY = self.dataY
        # 将两个数组合并到一起
        savePop = [[] for i in range(2)]
        savePop[0] = self.population_task1
        savePop[1] = self.population_task2

        # 用于保存探索解
        exploreSolution = [[] for i in range(self.populationNum)]

        # 探索阶段
        if runTime % self.exStage == 0:
            print(" 进入探索阶段 ")
            for i in range(2):
                currentFrequency, currentAccLen = self.getFeatureFrequency(population=savePop[i])
                self.currentSave.saveData(freTask=currentFrequency, acclentask=currentAccLen, task=i)
            # 进行计算
            rule = [[] for i in range(2)]
            # 因为任务1 是pro
            rule[0] = copy.deepcopy(self.probabilityOfFeatureToSelect)
            # 任务2 是relifF
            rule[1] = copy.deepcopy(self.scoreOfRelifF)
            for i in range(2):
                solutionArray = np.asarray([])
                accArray = np.asarray([])
                # 生成相对应长度的 初始新解
                newSArray = np.random.rand(self.populationNum * self.eliteProb, self.dataFeature.shape[0])
                for j in range(len(newSArray)):
                    # 满足条件就是1 否则就是0
                    newSolution = np.where(newSArray[j] < self.currentSave.frequency[i] * rule[i], 1, 0)
                    # 相当于每一次迭代 找出当前迭代过程中最高acc 和 最高acc所对应的特征组合
                    goodAcc = np.zeros(int(1 / self.eliteProb))
                    goodSolution = [[] for i in range(int(1 / self.eliteProb))]
                    # 用 每一个原始解析的长度来探索新的解，生成一些相同长度的新的解，判断这些新的解和原始解的差距，
                    # 要是差距不大就保留下来，而且相当于探索了新的解系，拥有了更多的方向
                    # self.populationNum 太大  少一点减少计算量
                    for m in range(int(1 / self.eliteProb)):
                        # 新解中 变成1 的索引
                        indexArray = np.where(newSolution == 1)[0].tolist()
                        # 获得 当前解析为 1 的数量
                        oriIndexLen = savePop[i][j].numberOfSolution
                        # 如果当前选择的长度比原始长度还大，则要将选择长度设置为原始长度减一
                        if oriIndexLen > len(indexArray):
                            oriIndexLen = len(indexArray) + 1
                        # 随机选择特征
                        newIndexArray = sample(indexArray, oriIndexLen)
                        # 获得其dataX
                        featureX = self.dataX[:, newIndexArray]
                        # 获得新解的acc
                        goodAcc[m] = fitnessFunction_KNN_CV(findData_x=featureX, findData_y=featureY, CV=5)
                        # 保存解系
                        temp1 = np.zeros(len(self.dataFeature))
                        temp1[newIndexArray] = 1
                        goodSolution[m] = temp1

                    # 降序排序acc
                    accIndex = np.argsort(goodAcc)[::-1]
                    # 排序acc 和 solution
                    goodAcc = goodAcc[accIndex]
                    goodSolution = np.asarray(goodSolution)
                    goodSolution = goodSolution[accIndex]
                    # # 如果第一个值要比原始值要小，则只需要保留第一个值即可
                    # oriAcc = 1 - savePop[i][j].mineFitness
                    # if goodAcc[0] < oriAcc:
                    #     accArray = np.append(accArray, goodAcc[0])
                    #     solutionArray = np.append(solutionArray, goodSolution[0])
                    # else:
                    #     # 将 goodAcc中所有比原始fitness要高的acc都保留，顺便保留solution
                    #     l = 0
                    #     while l < len(goodAcc) and goodAcc[l] > oriAcc:
                    #         accArray = np.append(accArray, goodAcc[l])
                    #         solutionArray = np.append(solutionArray, goodSolution[l])
                    #         l += 1
                    accArray = np.append(accArray, goodAcc)
                    solutionArray = np.append(solutionArray, goodSolution)


                # 排序 accArray --- 由高到低
                accIndex = np.argsort(accArray)[::-1]
                accArray = accArray[accIndex]
                solutionArray = solutionArray[accIndex]

                # 将所有的二进制solution取出，组合在一起
                sBin = [[] for l in range(self.populationNum)]
                sAcc = [[] for l in range(self.populationNum)]
                for l in range(self.populationNum):
                    sBin[l] = savePop[i][l].featureOfSelect
                    sAcc[l] = savePop[i][l].mineFitness

                # 将sAcc进行排序，将后20%的个体进行修改
                accIndex = np.argsort(sAcc)[::-1]
                for l in range(int(len(accIndex) * 0.8 ),len(accIndex)):
                    savePop[i][accIndex[l]].mineFitness = accArray[l]
                    savePop[i][accIndex[l]].featureOfSelect = solutionArray[l]
                    savePop[i][accIndex[l]].getNumberOfSolution()
                    # 根据 solution 来更新 solutionNum
                    savePop[i][accIndex[l]].featureOfSelectNum = np.where(savePop[i][accIndex[l]].featureOfSelect == 1,
                                                                               np.random.uniform(0, rule[i]),
                                                                               np.random.uniform(rule[i], 1))
                # 保存 去掉重复解 的数据集
                # 获得提取后的索引
                uniIndex = np.unique(sBin,axis=0, return_index=True)[1]

                # 生成新的解集
                if len(uniIndex) < self.populationNum:
                    # 需要添加多少个新的个体
                    needNum = self.populationNum - len(uniIndex)
                    print("needNum = ",needNum)
                    delArray = np.asarray([])
                    delIndex = np.asarray([])
                    for t in range(self.populationNum):
                        # 如果l不在该种群中，就需要生成一个新的解
                        if t not in uniIndex:
                            delArray = np.append(delArray,savePop[i][t])
                            delIndex = np.append(delIndex,t)
                    print(" 去重后的填充 ")

                    # 将重复的个体替换
                    for l in range(delIndex.shape[0]):
                        savePop[i][int(delIndex[l])].mineFitness = accArray[l]
                        savePop[i][int(delIndex[l])].featureOfSelect = solutionArray[l]
                        savePop[i][int(delIndex[l])].getNumberOfSolution()
                        # 根据 solution 来更新 solutionNum
                        savePop[i][int(delIndex[l])].featureOfSelectNum = np.where(savePop[i][l].featureOfSelect == 1,
                                                                                   np.random.uniform(0, rule[i]),
                                                                                   np.random.uniform(rule[i], 1))

            print()
            # 根据特征频率来考虑特征的选择问题，频率高的特征就有限被选中，然后组合其他一些频率较低的特征，形成新的
            # 解系，通过判断不同解的效果来进行计算
