...
from mfmode.operators import getFreq
...
class MFMODE2:
    ...
    # 设置参数
    def setParameter(self, populationNum, iteratorTime, eliteProb,crossoverPro, F,eliteFront,p):
    ...
    def mate_new(self,taskNum):
        ...
        # 将两个任务种群添加到一起
        taskPopulation = np.concatenate((self.population_task1, self.population_task1), axis=0)
    ...
    def goRun(self):
        ...
        # 写入文件的路径
        path_txt = "../result/paretoSolution/EG_MFDE_V2/" + self.dataName + ".txt"
        # 写入时间
...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        # 写入文件的路径
        path_txt = "../result/result_txt/EG_MFDE_V2/" + ducName + ".txt"
        # 写入文件的标题
        ...
        print("获取文件数据", file)
        dataCsv.getData()
        genetic = MFMODE2(dataX=dataCsv.dataX, dataY=dataCsv.dataY,dataName=ducName)

        genetic.setParameter(populationNum=70, iteratorTime=100, eliteProb=0.1,crossoverPro=0.9
                             ,F=0.5,eliteFront = 0.01,p = 0.05)
