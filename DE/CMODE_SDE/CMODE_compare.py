...
from dataProcessing.NonDominate import nonDominatedSort, getPF, nonDominatedSort_PFAndPop
...
class CMODESDE:
    ...
    def getElietSet(self):
        ...
        eliteSet,bestIndividual = nonDominatedSort(parentPopulation=self.population_DE,fitnessArray=fitArray,solutionlengthArray=lenArray
                         ,populationNum=self.alpha)

        # 跟新全局最优个体
        if self.globalFitness > bestIndividual.mineFitness:
    ...
    def environmentalSelection(self,populaiton):
        ...
        self.population_DE,individual = nonDominatedSort(parentPopulation=popArray,fitnessArray=fitArray,solutionlengthArray=lenArray,
                                            populationNum=self.populationNum)
        # 跟新全局最优个体
        if self.globalFitness > individual.mineFitness:
        ...
        self.global_len = np.append(self.global_len, self.globalSolution / self.dataFeatureNum)
