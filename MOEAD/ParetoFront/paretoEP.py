...
import numpy as np
# 更新pareto解
def updatePF(EP,individual):
    # 先将这个新个体添加到 EP中
    EP = np.append(EP,individual)
    # 存放error的数组  --- [0,1]
    errorArray = np.zeros(EP.shape[0])
    # 存放len的数组  --- [0,1]
    lenArray = np.zeros(EP.shape[0])
    # 用于保存每一个个体的支配解集
    dominateFrontArray = []
    ...
    for i in range(EP.shape[0]):
        # 将每一个个体的 error 存储
        errorArray[i] = EP[i].mineFitness
        # 将每一个个体的 len 存储
        lenArray[i] = EP[i].proportionOfFeature
    ...
    parentPopulation = EP[sortErrorIndex]
